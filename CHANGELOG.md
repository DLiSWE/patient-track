...
- Added a Closed days calendar to the Tools page for holidays and other center-wide closures. Managers click a weekday to close it for every member (with an optional reason) and click again to reopen. Closed days are skipped everywhere expected service days are computed (service calendar, bulk fill, Continue holds, status extend, expected/missed counts, attendance grid, claim review) and can't be marked in the service calendar; existing entries on a closed day are never removed. Adds/removes are logged as `closed_day_added` / `closed_day_removed`.
- Security / setup: new `closed_days` table (`supabase-closed-days.sql`). RLS: app users can read, only managers and super admins can insert or delete, no update policy, plus the same restrictive aal2 (MFA) gate as the other app tables. Run the file in the Supabase SQL editor before using the Closed days card.
